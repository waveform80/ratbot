...
class UserForm(TableForm):
    ...
    fields = [
        ...
        MultipleSelectField('groups', size=20, label_text='Groups', help_text='Select the groups this user belongs to'),
    ]
...
class GroupForm(TableForm):
    ...
    fields = [
        ...
        MultipleSelectField('users', size=20, label_text='Users', help_text='Select the users that belong to this group'),
        MultipleSelectField('permissions', size=20, label_text='Permissions', help_text='Select the permissions associated with this group'),
    ]
...
class PermissionForm(TableForm):
    ...
    fields = [
        ...
        Spacer(),
        TextField('display_name', size=50, validator=NotEmpty, label_text='Display name', help_text='Specify a display name for the permission'),
        MultipleSelectField('groups', size=20, label_text='Groups', help_text='Select the groups that are associated with this permission'),
    ]

new_user_form = UserForm('new_user_form', action=url('/admin/insert_user'))
new_group_form = UserForm('new_group_form', action=url('/admin/insert_group'))
new_permission_form = UserForm('new_permission_form', action=url('/admin/insert_permission'))
alter_user_form = UserForm('alter_user_form', action=url('/admin/update_user'))
alter_group_form = UserForm('alter_group_form', action=url('/admin/update_group'))
alter_permission_form = UserForm('alter_permission_form', action=url('/admin/update_permission'))
